...
-export([is_oahs/1, new/0, add/2, remove/2, contains/2, fold/3, filter/2, map/2, equal/2, sum/2, from_list/1, to_list/1]).
...
probe_slots(Array, Value, Slot, ProbeFun, N) ->
    ...
    case array:get(Slot, Array) of
        ...
        Value ->
            {found, Slot};
        _ ->
            probe_slots(Array, Value, ProbeFun(Slot, Value, N) rem array:size(Array), ProbeFun, N + 1)
    end.
    ...
    #oahs{
        array =
            case (Used / array:size(Array)) > ?LOAD_FACTOR of
                true ->
                    rehash(Array, array:new([{size, array:size(Array) * ?GROW_FACTOR}, {fixed, true}]));
                _ ->
                    Array
            end,
        used = Used
...
fold(Fun, Acc, #oahs{array = Array, used = _}) ->
    array:sparse_foldl(fun (_, Value, Acc_) -> Fun(Value, Acc_) end, Acc, Array).
